<?php
function ingenious_dimox_breadcrumbs() {
	/* === OPTIONS === */
	$text['home']	 = esc_html__( 'Home', 'ingenious' ); // text for the 'Home' link
	$text['category'] = esc_html__( 'Archive by Category "%s"', 'ingenious' ); // text for a category page
	...
	$delimiter	  = '<i class="delimiter">/</i>'; // delimiter between crumbs
	...
	$show_breadcrumbs = ingenious_get_option( 'show_breadcrumbs' );
	...
	if ( $show_breadcrumbs ) {
		if ( is_home() || is_front_page() || (empty( $post )) ) {
			...
			if ( get_query_var( 'paged' ) ) {
				...
				echo sprintf("%s", $delimiter) . esc_html__( 'Page', 'ingenious' ) . ' ' . get_query_var( 'paged' );
				if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) { echo ')'; }
			}
