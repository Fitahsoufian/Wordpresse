<?php
function ingenious_searchPostsGrid_post (){
	$pid = get_the_id();
	echo "<article id='post_post_{$pid}' class='item post post_post post_grid_post clearfix'>";
		
		ingenious_searchPostsGrid_postMedia ();

		ingenious_searchPostsGrid_postTitle ();

		ingenious_searchPostsGrid_postContent ();
		
		ingenious_searchPostsGrid_postTerms ();

		echo "<hr class='posts_grid_divider' />";											
	echo "</article>";	
}
function ingenious_searchPostsGrid_postMedia (){
	$pid = get_the_id();
	...
	if ( has_post_thumbnail() ) {
		...
		$thumb_obj = ingenious_thumb( $thumbnail, array( 'width' => 150, 'height' => 150 ), $thumb_id );
...
function ingenious_searchPostsGrid_postTitle (){
	$search_terms = get_query_var( 'search_terms' );	
	if ( empty( $search_terms ) ){
		ingenious_searchPostsGrid_postTitle_emptyQuery ();	
	}
	else{
		ingenious_searchPostsGrid_postTitle_searchByTerms ();
	}
}
function ingenious_searchPostsGrid_postTitle_emptyQuery (){
	$pid = get_the_id();
	$title = esc_html( get_the_title() );
	$permalink = esc_url( get_the_permalink() );
	echo !empty( $title ) ?	"<h3 class='post_post_title post_title'><a href='$permalink'>" . $title . "</a></h3>" : "";	
}
function ingenious_searchPostsGrid_postTitle_searchByTerms (){
	$search_terms = get_query_var( 'search_terms' );
	$title = esc_html( get_the_title() );
	$permalink = esc_url( get_the_permalink() );
...
function ingenious_postPostsGrid_getCharsCount() {
	$layout = "small";
	$number 	= 300;
...
function ingenious_searchPostsGrid_postContent (){
	$search_terms = get_query_var( 'search_terms' );	
	if ( empty( $search_terms ) ){
		ingenious_searchPostsGrid_postContent_emptyQuery ();	
	}
	else{
		ingenious_searchPostsGrid_postContent_searchByTerms ();
	}
}
function ingenious_searchPostsGrid_postContent_emptyQuery (){
	global $post;
	global $more;
	$id = get_the_ID();
	...
	extract( $grid_atts );
	$terms = ingenious_searchPostsGrid_get_postTerms ();
	$content = $proc_content = $excerpt = $proc_excerpt = "";
	$read_more_exists = false;
	$read_more = empty( $terms ) ? esc_html__( "Read More ", 'ingenious' ) . "<i class='fa fa-angle-right'></i>" : "<i class='fa fa-angle-double-right'></i>";
	...
	if ( !empty( $excerpt ) ){
		...
		if ( !empty( $content ) ){
			...
			$def_chars_count = ingenious_postPostsGrid_getCharsCount();
...
function ingenious_searchPostsGrid_postContent_searchByTerms (){
	global $post;
	$search_terms = get_query_var( 'search_terms' );
	$permalink = esc_url( get_the_permalink() );
	$terms = ingenious_searchPostsGrid_get_postTerms ();
	...
	if ( !empty( $cont ) ){
		echo "<div class='post_content post_post_content post_posts_grid_post_content clearfix" . ( !empty( $terms ) ? " read_more_alt" : "" ) . "'>";
			...
			if ( empty( $terms ) ){
				$cont .= esc_html__( "Read More", 'ingenious' ) . " <i class='fa fa-angle-right'></i>";
			}
			else{
...
function ingenious_searchPostsGrid_get_postTerms (){
	$pid = get_the_id();
	$post_type = get_post_type( $pid );
	$terms = $tags = $cats = $pcats = $deps = $poss = "";
	if ( $post_type == 'cwsportfolio' ){
		$pcats = ingenious_get_post_term_links_str( 'cwsportfolio-cat' );
		$terms .= !empty( $pcats ) ? "<i class='fa fa-bookmark'></i>&#x20;$pcats" : "";
	}
	else if ( $post_type == 'cwsstaff' ){
		...
		$terms .= !empty( $tags ) ? ( ( !empty( $tags ) ? INGENIOUS_V_SEP : "" ) . $tags ) : "";
	}
	return $terms;	
}
function ingenious_searchPostsGrid_postTerms (){
	$terms = ingenious_searchPostsGrid_get_postTerms ();
	if ( !empty( $terms ) ){
